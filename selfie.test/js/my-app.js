...
jQuery(document).ready(function($){
 ...
 myApp.onPageInit('splash', function (page) {
				   ...
				   console.log(number);
				    setTimeout(function(){
 document.getElementById("newimg").src="https://saltydogcafe.com/selfie.test/src/image-" + number + ".jpg";
    },1000);
	...
	$('iframe').attr('src','https://saltydogcafe.com/selfie.test/selfie/dev/dl.php?image=https://saltydogcafe.com/selfie.test/src/image-' + number + '.jpg');
...
$('.discard').on('click',function(e){
	e.preventDefault();     
mainView.router.loadPage('https://saltydogcafe.com/selfie.test/');
...
document.getElementById("photo").src="https://saltydogcafe.com/selfie.test/src/image-" + number + ".jpg";
...
function test4(){
	...
	$.ajax({
           type: "POST",
           url: 'https://saltydogcafe.com/selfie.test/test4.php',
...
function test6(){
	...
	$.ajax({
           type: "POST",
           url: 'https://saltydogcafe.com/selfie.test/test6.php',
           data: $("#webshot").serialize(), // serializes the form's elements.
           success: function(data)
           {
			 console.log(data);
...
function test2(){
	var url = 'https://saltydogcafe.com/selfie.test/test2.php';
    var postData = $('#webshot').serialize();
	var message = $('textarea[name=message]').value;
	if ($('.message').val().length === 0 ){
		$('.message').val('No Message');
...
$('.cancel').on('click',function(e){
	e.preventDefault();     
mainView.router.loadPage('https://saltydogcafe.com/selfie.test/selfie/dev/index.php');
});
jQuery(document).ready(function(){
	$("#webshot").on('click','#submit',function(e)
{
	e.preventDefault();
	mainView.router.loadPage('https://saltydogcafe.com/selfie.test/selfie/dev/index.php');
    var url = 'https://saltydogcafe.com/selfie.test/test3.php';
    var postData = $('#webshot').serialize();
    var formURL = url;
    $.ajax({
           type: "POST",
